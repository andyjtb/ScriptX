...
InvalidateCacheOnDestroy::~InvalidateCacheOnDestroy() { propNameIDCache.invalidate(key); }
...
const jsi::PropNameID &PropNameIDCache::getConstructorNameProp(jsi::Runtime &runtime,
  ...
  switch (kind) {
    ...
    case TypedArrayKind::DataView:
      return get(runtime, Prop::DataView);
  }
}
...
std::unordered_map<std::string, TypedArrayKind> nameToKindMap = {
    ...
    {"DataView", TypedArrayKind::DataView},
};

TypedArrayKind getTypedArrayKindForName(const std::string &name) { return nameToKindMap.at(name); }
